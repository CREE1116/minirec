...
def _evaluate_full(model, test_loader, top_k_list, metrics_list, device,
    ...
    if item_popularity is not None and needed_groups:
        ...
        for g in needed_groups:
            ...
            group_masks[g] = mask
            if g == 'Tail': group_masks['LongTail'] = mask
        ...
        for start in tqdm(range(0, n_users, batch_size), desc="Eval (GPU)", file=sys.stdout):
            ...
            for k in top_k_list:
                ...
                for g, mask in group_masks.items():
                    g_gt = gt_dense_batch & mask
                    g_sz = g_gt.sum(dim=1).float(); v_mask = g_sz > 0; n_v = v_mask.sum().item()
                    if n_v > 0:
                        h_v_batch = torch.gather(g_gt[v_mask].float(), 1, top_idx[v_mask, :k])
                        h_sum = h_v_batch.sum(dim=1)
                        sums[f'{g}HitRate@{k}'] += (h_sum > 0).float().sum().item()
                        sums[f'{g}Recall@{k}'] += (h_sum / g_sz[v_mask]).sum().item()
                        dcg_v = (h_v_batch * log2_w[:k]).sum(dim=1)
                        sums[f'{g}NDCG@{k}'] += (dcg_v / idcg_lut[g_sz[v_mask].clamp(max=k).long()].clamp(min=1e-8)).sum().item()
                        counts[f'{g}HitRate@{k}'] += n_v
                        counts[f'{g}Recall@{k}'] += n_v
                        counts[f'{g}NDCG@{k}'] += n_v
