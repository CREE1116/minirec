...
class CausalAspire(BaseModel):
    ...
    def fit(self, data_loader):
        print(f"Fitting Causal ASPIRE (alpha={self.alpha}) on CPU (Simple Style)...")
        ...
        item_bias = np.asarray(X_sp.T.dot(u_weights)).ravel().astype(np.float32)
        i_weights = (np.float32(1.0) / np.sqrt(item_bias + self.eps)).astype(np.float32)
        ...
        W_np = (-P_np / (diag_P + self.eps)).astype(np.float32)
