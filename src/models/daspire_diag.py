...
class DAspireDiag(BaseModel):
    """Ablation: DAspire (DLAE-style) logic using only Diagonal (d_i) instead of SNR."""
    ...
    def fit(self, data_loader):
        print(f"Fitting DAspireDiag (DLAE-style, d_i scaling), lambda={self.reg_lambda}, alpha={self.alpha}, p={self.dropout_p})")
        ...
        reliability = (log_d - log_d.mean()).exp()
