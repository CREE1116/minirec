...
class LightGCN(BaseModel):
    ...
    def _get_norm_adj(self, data_loader):
        # A_hat = D^-0.5 * A * D^-0.5
        adj = data_loader.get_interaction_graph(add_self_loops=False).to(self.device)
        ...
        return torch.sparse_coo_tensor(indices, values, adj.shape).to(self.device)
    ...
    def calc_loss(self, batch_data):
        ...
        # Reg loss
        reg_loss = self.reg_weight * (self.user_embedding(u_idx).norm(2).pow(2) + 
                                     self.item_embedding(p_idx).norm(2).pow(2) + 
                                     self.item_embedding(n_idx).norm(2).pow(2)) / len(u_idx)
