...
class MF(BaseModel):
    ...
    def forward(self, user_indices):
        ...
        scores = torch.matmul(u_emb, self.item_emb.weight.t()) # (Batch, N_Items)
        return scores

    def calc_loss(self, batch_data):
        # batch_data: {'user_id': (B, 1), 'pos_item_id': (B, 1), 'neg_item_id': (B, N)}
        ...
        n_e = self.item_emb(n_idx) # (B, N, Dim)

        pos_scores = (u_e * p_e).sum(dim=-1) # (B,)
        
        if n_e.dim() == 3: # Multiple negatives
            u_e_expanded = u_e.unsqueeze(1) # (B, 1, Dim)
            neg_scores = (u_e_expanded * n_e).sum(dim=-1) # (B, N)
