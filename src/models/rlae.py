...
class RLAE(BaseModel):
    ...
    def fit(self, data_loader):
        print(f"Fitting RLAE (lambda={self.reg_lambda}, b={self.b}) on CPU (Minimal Copy)...")
        ...
        print(f"  inverting matrix (NumPy)...")
        P = np.linalg.inv(A)
        del A
        gc.collect()
        
        diag_P = np.diag(P)
        penalty = np.maximum(self.reg_lambda, (np.float32(1.0) - self.b) / (diag_P + self.eps))
        
        # In-place weight calc
        P *= -penalty[np.newaxis, :]
        P[np.diag_indices_from(P)] += np.float32(1.0)
        
        self.weight_matrix = torch.tensor(P, dtype=torch.float32, device=self.device)
        del P
...
class RDLAE(BaseModel):
    ...
    def fit(self, data_loader):
        print(f"Fitting RDLAE (lambda={self.reg_lambda}) on CPU (Minimal Copy)...")
        X_sp = get_train_matrix_scipy(data_loader)
        self.train_matrix_cpu = X_sp.tocsr()
        G_np = compute_gram_matrix(X_sp, data_loader)
        gc.collect()
        ...
        p = self.dropout_p
        dropout_penalty_np = ((p / (np.float32(1.0) - p)) * np.diag(G_np))
        ...
        diag_P = np.diag(P)
        total_penalty = np.maximum(lambda_diag_np, (np.float32(1.0) - self.b) / (diag_P + self.eps))
        
        # In-place weight calc
        P *= -total_penalty[np.newaxis, :]
        P[np.diag_indices_from(P)] += np.float32(1.0)
